...
function checkJiaohangInvoiceLoan(answers) {
  return (
    answers.company_age >= 1 &&                    // 成立≥1年
    answers.annual_invoice_amount_min >= 1000000 && // 年开票≥100万
    answers.invoice_duration === "more_than_12m" && // 连续开票≥12月
    answers.industry !== "real_estate" &&           // 非房地产
    !hasSeriousCreditIssue(answers)                 // 无严重征信问题
  );
}
```
...
function checkPinganInvoiceLoan(answers) {
  return (
    ...
    answers.annual_invoice_amount_min >= 2000000 && // 年开票≥200万
    answers.invoice_duration !== "less_than_6m" &&  // 连续开票≥6月
...
function checkZhaohangInvoiceLoan(answers) {
  return (
    answers.company_age >= 1 &&
    answers.annual_invoice_amount_min >= 1500000 && // 年开票≥150万
...
function checkJianhangTaxLoan(answers) {
  return (
    answers.company_age >= 1 &&
    answers.annual_tax_amount_min >= 20000 &&       // 年纳税≥2万
    ["level_a", "level_b"].includes(answers.tax_credit_level) && // A/B级
    !hasSeriousCreditIssue(answers)
  );
}
```
...
function checkZhongguoTaxLoan(answers) {
  return (
    answers.company_age >= 1 &&
    answers.annual_tax_amount_min >= 30000 &&       // 年纳税≥3万
...
function checkGonghangTaxLoan(answers) {
  return (
    answers.company_age >= 2 &&                     // 成立≥2年
    answers.annual_tax_amount_min >= 10000 &&       // 年纳税≥1万
    answers.tax_credit_level !== "level_d" &&       // 非D级
    !hasSeriousCreditIssue(answers)
  );
}
```
...
function checkZhongguoMortgageLoan(answers) {
  return (
    answers.has_property === true &&                // 有房产
    answers.property_value_min >= 1000000 &&        // 房产估值≥100万
    !hasSeriousCreditIssue(answers)
  );
}
```
...
function checkGonghangQuickLoan(answers) {
  return (
    ...
    (answers.annual_invoice_amount !== "no_invoice" ||
     answers.annual_tax_amount_min >= 10000) &&     // 有开票或有纳税
    !hasSeriousCreditIssue(answers)
  );
}
```
...
function checkWeizhongBusinessLoan(answers) {
  return (
    answers.company_age >= 1 &&
    answers.annual_revenue_min >= 1000000 &&        // 年营收≥100万
    !hasSeriousCreditIssue(answers)
  );
}
```
...
function calculateInvoiceLoanAmount(answers, baseRate) {
  let coefficient = baseRate; // 基础系数（5%-15%，视银行而定）
  ...
  // 银行额度上限
  const maxLimit = 3000000; // 300万（视具体产品而定）
...
function calculateTaxLoanAmount(answers, baseMultiplier) {
  let multiplier = baseMultiplier; // 基础倍数（10-20倍，视银行而定）
  ...
  // 银行额度上限
  const maxLimit = 3000000; // 300万（建行云税贷）或5000000（工行税务贷）
  amount = Math.min(amount, maxLimit);
...
function calculateHighTechLoanAmount(answers) {
  ...
  if (answers.has_patent === "invention") {
    amount += 750000; // +75万（取中值）
  } else if (answers.has_patent === "utility") {
    amount += 350000; // +35万（取中值）
  }
  
  // 股权投资加分（MVP版本暂不实现，问卷未收集）
...
| 类型 | 个人创业 | 企业 | 说明 |
|------|---------|------|------|
| 大学生创业 | 最高30万 | 最高400万 | 财政贴息 |
| 退伍军人创业 | 最高30万 | 最高500万 | 财政贴息 |
...
function calculateQuickLoanAmount(answers) {
  ...
  // 额度映射
  let amount = 0;
  if (score >= 80) amount = 5000000;      // 500万
  else if (score >= 60) amount = 3000000; // 300万
  else if (score >= 40) amount = 1000000; // 100万
  else if (score >= 20) amount = 500000;  // 50万
  else amount = 300000;                   // 30万（最低额度）
...
| 缺口类型 | 说明 | 对应方案 |
|---------|------|---------|
| `invoice_insufficient` | 开票额不足 | 开票提升术 |
| `tax_insufficient` | 纳税额不足 | 纳税提升术 |
| `tax_level_low` | 纳税等级低 | 纳税提升术 |
| `no_property` | 无抵押物 | 资产增持术 |
| `no_qualification` | 无特殊资质 | 资质认定术 |
| `company_too_young` | 企业年限不足 | 时间等待（无方案） |
...
function calculateSchemeROI(gap, scheme) {
  ...
  return {
    ...
    cost: cost,
    roi: roi.toFixed(2) + "%",
    payback_months: Math.ceil(cost / (expectedIncrease * 0.05 / 12)) // 假设年化利息5%
  };
}
```
...
function diagnoseAndRecommend(questionnaireAnswers) {
  ...
  return {
    ...
    gaps: gaps.slice(0, 3), // 最多展示3个缺口
    recommended_schemes: rankedSchemes.slice(0, 3), // 最多推荐3个方案
...
function normalizeAnswers(raw) {
  return {
    ...
    annual_invoice_amount_min: getNumericValue(raw.annual_invoice_amount, "amount"),
    annual_tax_amount_min: getNumericValue(raw.annual_tax_amount, "amount"),
...
{
  "company_age": "1_to_2",
  "annual_invoice_amount": "less_than_100w",
  "invoice_duration": "less_than_6m",
  "annual_tax_amount": "1w_to_5w",
...
```json
{
  "current_status": {
    ...
    "recommended_products": [
      {
        "product_name": "新网银行-好企贷",
        "amount": 500000,
        "rate_avg": 0.108
      },
      {
        "product_name": "微众银行-微业贷",
        "amount": 300000,
        "rate_avg": 0.072
      }
    ]
...
{
  "company_age": "1_to_2",
  "annual_invoice_amount": "300w_to_500w",
  "invoice_duration": "more_than_12m",
  "annual_tax_amount": "5w_to_10w",
...
```json
{
  "current_status": {
    ...
    "recommended_products": [
      {
        "product_name": "邮储银行-科创贷",
        "amount": 3750000,
        "rate_avg": 0.045
      },
      {
        "product_name": "大学生创业贷-企业",
        "amount": 4000000,
        "rate_avg": 0.02
      },
      {
        "product_name": "招商银行-发票贷",
        "amount": 600000,
        "rate_avg": 0.0675
      }
    ]
...
| 用例 | 场景 | 期望匹配产品数 | 期望缺口数 | 主推方案 |
|------|------|--------------|----------|---------|
| 4 | 无开票无纳税-新公司 | 0-1 | 2-3 | 开票提升术 |
| 5 | 高开票-低纳税 | 3-4 | 1 | 纳税提升术 |
| 6 | 低开票-高纳税 | 2-3 | 1 | 开票提升术 |
| 7 | 有房产-无开票 | 2 | 1 | 开票提升术 |
| 8 | 专精特新-无专利 | 2-3 | 0 | 无需改造 |
| 9 | 退伍军人-零基础 | 2 | 0 | 优先特殊人群贷 |
| 10 | 高额度-高利率-有贷款 | 5+ | 0 | B业务（降成本） |
...
| 版本 | 日期 | 修改内容 | 作者 |
|------|------|---------|------|
| v1.0 | 2025-12-06 | 初版，完整算法设计 | Claude |

---

**文档状态**：✅ 已完成  
**交付标准**：后端开发人员可直接按此文档实现匹配算法  
**下一步**：创建"项目进度追踪器.md"
